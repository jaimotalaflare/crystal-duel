...
function legacyDeckList(element,heavy=MAGES[element].heavy){return CATALOG[element].flatMap(c=>{const siblings=CATALOG[element].filter(x=>x.cost===c.cost),i=siblings.indexOf(c);let n=c.cost<=4?2:c.cost===5?(i===2?1:2):c.cost===6?(i===0?2:1):c.cost===7?1:c.cost===10||c.cost===heavy?1:0;return Array.from({length:n},()=>({...c,kw:[...(c.kw||[])]}))});}
...
export function deckList(element,heavy=8){const c=CATALOG[element];return [0,0,2,2,4,4,5,6,6,7,9,9,12,14,heavy===9?17:16].map(i=>({...c[i],kw:[...(c[i].kw||[])]}))}
